...
class LowerLevelObjective(nn.Module):
    ...
    def forward(self, x, theta):
        return (
            ...
            + 0.5 * torch.exp(theta[0]) * self.TV2D(x, torch.exp(theta[1]))
        ) / self.y.shape[0]
...
def FISTA(self, theta, x, tol, max_iter):
    ...
    for k in range(max_iter):
        ...
        self.mu = 1
        mu = self.mu
        L = 1 + (torch.exp(theta[0]) / torch.exp(theta[1])) * torch.sqrt(
            torch.tensor(8)
        )
        tau = 1 / L
        q = tau * mu
...
# device = torch.device("mps") # Uncomment if you use Apple devices
...
# Running different settings of accuracy and initial step size
