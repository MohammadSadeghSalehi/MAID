...
class MAID(nn.Module):
    ...
    def bound(self, x, theta, jac, hess, epsilon):
        ...
        c = (
            Bx / mu
            + (self.LAinv) * torch.linalg.norm(self.grad_upper(x)) * Bx
            + self.LB * torch.linalg.norm(self.grad_upper(x)) / mu
        )
    ...
    def backtrack(self, x, theta, p, epsilon, beta, rho, tau, max_iter=10):
        for i in range(max_iter):
            ...
            if (
                g
                + torch.linalg.norm(self.grad_upper(x_new)) * epsilon
                + self.Lg * epsilon**2 / 2
                - g_old
                + torch.linalg.norm(self.grad_upper(x)) * epsilon
                ...
                if self.psnr_log:
                    print(
                        ...
                        g,
                        "psnr = ",
                        psnr(x_new, self.upper_level_obj.x_true),
                    )
                else:
        ...
        for k in range(self.max_iter):
            ...
            if not self.fixed_eps:
                for j in range(self.max_bt_loop):
                    p, epsilon, delta = self.inexact_gradient(
                    ...
                    self.x, self.theta = self.backtrack(
                        self.x.requires_grad_(True),
                        self.theta.requires_grad_(True),
                        p,
                        epsilon,
