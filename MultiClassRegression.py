...
class LowerLevelObjective(nn.Module):
    ...
    def forward(self, x, theta):
        return nn.CrossEntropyLoss()(self.a@x, torch.argmax(self.b, dim=1))+ 0.5 * torch.sum(torch.dot(torch.exp(theta),x.flatten()**2)) 
...
def FISTA(self, theta, x, tol, max_iter):
    ...
    for k in range(max_iter):
        ...
        if torch.isnan(self.upper_level_obj(z - tau * p)) or torch.linalg.norm(p) / mu > 1e5:
            return x.detach()
...
class HOAG(nn.Module):
    def __init__(self,theta, x0, lower_level_obj, upper_level_obj, Lg, max_iter = 100 ,epsilon = 1e-2 , rho = 0.9, save_postfix= "", accuracy_type = "geometric") -> None:
        super().__init__()
        self.theta = theta
        self.x = x0 
    ...
    def main(self):
            ...
            for k in range(self.max_iter):
                ...
                if g_func <= g_func_old + C * epsilon + \
                                old_epsilon * (C + factor_L_lambda) * incr - factor_L_lambda * (L_lambda) * incr * incr :
                    L_lambda *= 0.95
                    step_size = 1/L_lambda
            ...
            self.state['loss'] = self.loss
            self.state['params'] = self.theta
            torch.save(self.state, os. getcwd()+ f"/state_dict_HOAG{self.accuracy_type+self.save_postfix}.pt")
            return self.theta
...
theta = -3 * torch.ones((num_classes, input_size), requires_grad=True).flatten().to(device)
...
for _ in range(10):
    ...
    grad_x = torch.autograd.grad(outputs=out, inputs=x, grad_outputs=None, create_graph=True, retain_graph=True, only_inputs=True, allow_unused=True)[0]
    hvp = torch.autograd.grad(outputs=grad_x, inputs=x, grad_outputs=d, create_graph=True, retain_graph=True, only_inputs=True, allow_unused=True)[0]
...
for index, acc in enumerate(["geometric", "cubic", "quadratic"]):
    model = HOAG(theta, x0, lower_level_obj, upper_level_obj, Lg, max_iter=500, epsilon= 1e-1, rho=0.9 , save_postfix = "",accuracy_type= acc).to(device)
    model.main()
...
for index, postfix in enumerate(["0101"]):
    if index == 0:
        ...
        model = MAID(theta, x0, lower_level_obj, upper_level_obj, FISTA, Lg, max_iter=500, epsilon=1e-1, delta=1e-1, beta=init_step_size,save_postfix=postfix, jac = jacobian, jact= jacobiantrans).to(device)
    model.main()
