...
temp = (-A1 @ torch.inverse(A2.T @ A2) @ A2.T @ A3).T @ (
    A1 @ (-torch.inverse(A2.T @ A2) @ A2.T @ A3)
)

optimal = torch.inverse(temp) @ (
    -(-A1 @ torch.inverse(A2.T @ A2) @ A2.T @ A3).T
    @ (A1 @ torch.inverse(A2.T @ A2) @ A2.T @ (b2) - b1)
)
...
def jacobian(self, x, theta, d):
    ...
    jvp = torch.autograd.grad(
        outputs=gradvp,
        inputs=theta,
        grad_outputs=torch.ones(gradvp.shape).to(device).requires_grad_(True),
        create_graph=True,
        retain_graph=True,
        only_inputs=True,
        allow_unused=True,
...
for i in range(int(5e3)):
    ...
    temp = -A1 @ torch.inverse(A2.T @ A2) @ A2.T @ A3
    exact_hyper_gradient = (-A1 @ torch.inverse(A2.T @ A2) @ A2.T @ A3).T @ (
        A1 @ x_star - b1
    )
...
for index, postfix in enumerate(
    ["0303", "0505", "0101", "0303_fixed", "0505_fixed", "0101_fixed"]
):
    if index == 1:
        model = MAID(
            torch.ones(m),
