...
class FOE(nn.Module):
    def __init__(self, num_kernels, kernel_size):
        ...
        self.conv = nn.Conv2d(1, num_kernels, kernel_size, padding=2, bias=False,padding_mode= 'reflect')
    ...
    def forward(self, x, theta):
        with torch.no_grad(), torch.autograd.set_detect_anomaly(True):
            ...
            self.conv.weight.data = theta[self.num_kernels * 2 + 1:
                                           self.num_kernels * 2 + 1 + self.num_kernels * self.kernel_size**2].reshape(
                self.num_kernels, 1, self.kernel_size, self.kernel_size)
        x = self.conv(x)
        out = sum(
            torch.exp(self.params[i])
            * self.smoothed_norm1(x[:, i - 1], torch.exp(self.params[i + self.num_kernels]))
...
theta = -3 * torch.ones(2*num_filters+1+num_filters*kernel_size**2, requires_grad=True)
theta[num_filters:2*num_filters] = -3.5
for i in range(num_filters):
    theta[2*num_filters+1+i*kernel_size**2:2*num_filters+1+(i+1)*kernel_size**2] = torch.tensor(filters[i].flatten())
Lg = 1
...
with torch.no_grad(), torch.autograd.set_detect_anomaly(True):
    ...
    lower_level_obj.FOE.conv.weight.data = theta[lower_level_obj.FOE.num_kernels * 2 + 1:
                                    lower_level_obj.FOE.num_kernels * 2 + 1 + lower_level_obj.FOE.num_kernels * lower_level_obj.FOE.kernel_size**2].reshape(
        lower_level_obj.FOE.num_kernels, 1, lower_level_obj.FOE.kernel_size, lower_level_obj.FOE.kernel_size)
...
def FISTA(self,theta, x, tol, max_iter):
    ...
    for k in range(max_iter):
        x_old = x
        mu = (torch.exp(theta[0]) * sum((torch.exp(theta[i])/ (torch.exp(theta[i+num_filters]))) * torch.linalg.norm(theta[num_filters*2+1+i * kernel_size**2: num_filters*2+1+(i+1) * kernel_size**2])**2 for i in range(1,num_filters+1)))
        ...
        z = (x + beta * (x-x_old))
        p = torch.autograd.grad(outputs=self.lower_level_obj(z,theta), inputs=z, grad_outputs=None, create_graph=False, retain_graph=False, only_inputs=True, allow_unused=False)[0].detach()
